...
async def update_file(request: Request):  # 以覆盖方式更新文件
    try:
...
async def create_file(request: Request):  # 创建文件
    data = await request.json()
...
async def delete_file(request: Request):  # 删除文件
    data = await request.json()
    path = data["path"]
    os.remove(path)
    return {"success": True}
...
async def rename_file(request: Request):  # 重命名文件
    try:
        data = await request.json()
...
async def read_file(request: Request):  # 读取文件
    try:
        data = await request.json()
        path = data["path"]
        full_path = os.path.join(WORKSPACE_ROOT, path)
...
async def list_files_in_dir(rel_path: str):  # 列出目录下的文件
    try:
...
async def browse_filesystem(path: str = ""):
    ...
    try:
        ...
        return {
            "current_path": path,
            "parent_path": (
                os.path.dirname(path) if path != os.path.dirname(path) else None
            ),
            "items": items,
        }
